...
def _parse() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Envía logs sintéticos FortiGate (categoría traffic)")
    add_repo_arg(p)
    add_network_args(p)
    add_throughput_args(p)
    add_syslog_reporting_ip_arg(p)
    p.add_argument("--src-ip-mode", default="random", type=parse_src_ip_mode)
    p.add_argument("--attacker-ip", default="", help="IP en plantilla y origen del paquete UDP")
    p.add_argument("--fortigate-devname", default="")
    p.add_argument("--fortigate-serial", default="")
    p.add_argument("--c2-ip", default="", help="Destino sospechoso en plantillas traffic")

...
def _parse() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Envía logs sintéticos FortiEDR")
    add_repo_arg(p)
    add_network_args(p)
    ...
    p.add_argument(
        "--user-samaccountname",
        default="",
        help="Usuario Windows (users_ad.csv) para domain\\\\user en plantillas",
    )
    p.add_argument(
        "--endpoint-ip",
        default="",
        help="IP del endpoint (EDR); sintético si no está en assets.csv",
    )
    p.add_argument("--initial-asset-ip", default="", help="Alias de --endpoint-ip si prefieres el nombre del monolito")
    p.add_argument("--initial-asset-hostname", default="", help="Hostname del endpoint")
    p.add_argument("--hostname", default="", help="Hostname alternativo en eventos")
    p.add_argument(
        "--category",
        default="detection",
        help="Categoría en plantillas (detection, ransomware, ...)",
    )
...
def _main() -> int:
    ...
    ctx = build_send_context(
        ...
        active_sources=frozenset({"fortiedr"}),
        run_label="fortiedr",
    )
    total = run_sender(
        base,
